function [ms, biases] = mf_sepw_powcomb(d, p)
...
% mf_sepk_powcomb in the data folder. It uses those to find adequate
...
% w_vis (7 - 6+coh_num)
% b_comb (7+coh_num), g_comb (8+coh_num), gam_comb (9+coh_num)
% tnd_vis, tnd_vest, tnd_comb (10+coh_num - 12+coh_num)
% (2 x coh_num + 1) biases
% pl
%
% resulting in 3 x coh_num + 14 parameters.
%
...
if nargin < 2
    ...
    sepk_p = load(['fit_data' filesep 'mf_sepk_powcomb_' d.subj_id '.mat']);
    assert(c_num == length(sepk_p.cohs));
    sepk_p = sepk_p.best_p;
    % build initial data structure
    sepk_kvis = sepk_p(1)*cs.^sepk_p(4)./sqrt(1+sepk_p(2)*cs.^sepk_p(4));
    sepk_kvest = sepk_p(5);
    w_vis_ini = sepk_kvis.^2./(sepk_kvis.^2+sepk_kvest^2);
    sepk_p(7:(c_num+6)) = w_vis_ini;
    ms = struct();
    ms.p_min = [0.0001  0 0.001   0 0.00001 0.001      zeros(1,c_num)   0 0.001   0   0   0   0 (-5*ones(1,2*c_num+1))   0];
    ms.p_max = [ 10000 50    10  10    1000    10       ones(1,c_num) 200    10  10   1   1   1  (5*ones(1,2*c_num+1)) 0.2];
    ms.p_ini = sepk_p;
    ms.p_w   = [    50  1  0.05 0.5      20  0.03 (0.2*ones(1,c_num))  10  0.05 0.5 0.2 0.2 0.2    (ones(1,2*c_num+1)) 0.1];
    ms.p_names = cat(2,...
...
w_vis = p(7:(6+c_num));
w_vest = 1-w_vis;
bound_comb = p(8+c_num)./sqrt(1+p(7+c_num)*c.^p(9+c_num));
tnds = p((10+c_num):(12+c_num));
biases = struct('vis', p((13+c_num):(2*c_num+12)), 'vest', p(2*c_num+13), ...
    'comb', p((2*c_num+14):(3*c_num+13)));
pl = p(3*c_num+14);
