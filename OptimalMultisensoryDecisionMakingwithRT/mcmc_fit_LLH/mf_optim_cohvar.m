function [ms, biases] = mf_optim_cohvar(d, p)
%% returns the model predictions for given subject data and parameters
%
% this model implements optimal predictions, assuming that k's and bounds
% in the visual and combined condition are liked by a variance-based
% argument.
%
% the model models k and bound the following way:
%
% k_vis(c) = a_vis c^gamma / sqrt(1 + b_vis c)
% bound_vis(c) = g_vis / sqrt(1 + b_vis c)
% bound_comb(c) = g_comb / sqrt(1 + b_comb c)
%
% (alternative - not used)
% k_vis(c) = a_vis c^gamma / sqrt(b_vis + c)
% bound_vis(c) = g_vis / sqrt(b_vis + c)
% bound_comb(c) = g_comb / sqrt(b_comb + c)
%
...
%  7 - g_comb
%  8 - b_comb
%  9+- tnd_vis, tnd_vest, tnd_comb
% .. - (2 x coh_num + 1) biases
% .. - pl
%
% resuting in 13 + 2 x coh_num parameters
%
...
if nargin < 2
    c_num = length(d.cohs);
    ms = struct();
    ms.p_min = [0.00001 0.001   0 0.001 0.00001 0.001   0.001 0.001   0   0   0 (-5*ones(1,2*c_num+1))   0]; 
    ms.p_max = [  10000   100  10   100    1000     5     100   100   1   1   1  (5*ones(1,2*c_num+1)) 0.2];
    ms.p_ini = [    800     2   2   0.3     100   0.1       2     2 0.2 0.2 0.2     zeros(1,2*c_num+1)   0];
    ms.p_w   = [    400     2 0.3   0.3      50     1       2     2 0.2 0.2 0.2    (ones(1,2*c_num+1)) 0.1];
    ms.p_names = cat(2,{'avis','bvis','gamma','gvis','kvest','boundvest',...
        'gcomb','bcomb','tndvis','tndvest','tndcomb'},...
...
c = d.cohs;
c_num = length(c);
sig_vis = sqrt(1 + p(2)*c);
k_vis = p(1)*c.^p(3)./sig_vis;
bound_vis = p(4)./sig_vis;
k_vest = p(5);
bound_vest = p(6);
bound_comb = p(7)./sqrt(1+p(8)*c);
tnds = p(9:11);
biases = struct('vis', p(12:(11+c_num)), 'vest', p(12+c_num), ...
    'comb', p((13+c_num):(12+c_num*2)));
pl = p(2*c_num+13);
