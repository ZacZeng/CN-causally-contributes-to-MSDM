function [ms, biases] = mf_nocuetempw(d, p)
...
% models are summes according to x_vest = sqrt(2)*(x_vis + x_vest). 
%
% For initialisation, the model assumes to find the fits for
% mf_nocuetempw_powcomb in the data folder. It uses those to find adequate
...
if nargin < 2
    cs = d.cohs;
    c_num = length(cs);
    % load best-found parameters from nocuetempw_powcomb fit for initialisation
    prev_p = load(['fit_data' filesep 'mf_nocuetempw_powcomb_' d.subj_id '.mat']);
