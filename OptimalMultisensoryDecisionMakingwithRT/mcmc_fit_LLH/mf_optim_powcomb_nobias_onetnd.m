function [ms, biases] = mf_optim_powcomb_nobias_onetnd(d, p)
...
% Furthermore, it does not assume any biases, and a single, shared
% non-decision time.
%
% For initialisation, the model assumes to find the fits for
% mf_optim_powcomb in the data folder. It uses those to find adequate
...
if nargin < 2
    ...
    ms = struct();
    ms.p_min = [0.0001  0 0.001   0 0.00001 0.001   0 0.001   0   0   0];
    ms.p_max = [ 10000 50    10  10    1000    10 200    10  10   1 0.2];
    ms.p_ini = [prev_p(1:9) tnd_ini prev_p(end)];
    ms.p_w   = [    50  1  0.05 0.5      20  0.03  10  0.05 0.5 0.2 0.1];
    ms.p_names = cat(2,...
