function [ms, biases] = mf_accw(d, p)
...
% that both modalities are weighted by the acceleration.
%
...
if nargin < 2
    cs = d.cohs;
    c_num = length(cs);
    % load best-found parameters from accw_powcomb fit for initialisation
    prev_p = load(['fit_data' filesep 'mf_accw_powcomb_' d.subj_id '.mat']);
