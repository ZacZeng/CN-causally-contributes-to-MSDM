function [ms, biases] = mf_nocuew(d, p)
...
% it does not perform the correct cue weighting.
%
% For initialisation, the model assumes to find the fits for
% mf_nocuew_powcomb in the data folder. It uses those to find adequate
...
if nargin < 2
    cs = d.cohs;
    c_num = length(cs);
    % load best-found parameters from nocuew_powcomb fit for initialisation
    prev_p = load(['fit_data' filesep 'mf_nocuew_powcomb_' d.subj_id '.mat']);
