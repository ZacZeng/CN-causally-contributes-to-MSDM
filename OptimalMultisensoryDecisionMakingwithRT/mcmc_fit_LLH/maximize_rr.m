function maximize_rr(subj_id, restart_num, no_bias, model_name)
%% tunes bounds to maximize reward rate for given subject id
%
...
% to find the bounds that maximize the latter.
%
...
if on_cluster
    ...
    if nargin < 3, no_bias = false;
    else no_bias = logical(str2num(no_bias));
    end
    if nargin < 2, restart_num = 100;
    ...
    if nargin < 2, restart_num = 10; end
end
...
if no_bias
    rr_data_filename = [data_folder filesep 'max_rr_no_bias_' model_name '_' subj_id '.mat'];
else
    rr_data_filename = [data_folder filesep 'max_rr_' model_name '_' subj_id '.mat'];
end
...
R = [1 0];
iti = 6;
fprintf('Maximizing reward rate for subject %s, with %d random restarts\n', ...
    subj_id, restart_num);
...
rr_emp = rrate(dstats, dstats, R, iti);
rr_ini = rrate(dm.mstats, dstats, R, iti);
...
rr_rnd = (0.5*R(1)+0.5*R(2))/(mean_tnd+iti);
...
if no_bias
    ...
    p(bias_idx) = 0;
    rr_ini_no_bias = model_rrate(bp_ini, p, bp_idcs, ds, dstats, R, iti, model_fn);
    fprintf('model reward rate (no bias): %6.4f\n', rr_ini_no_bias);
end


%% reward-rate function
RR_func = @(bp) model_rrate(max(bp_min, min(bp_max, bp)),p,bp_idcs,ds,dstats,R,iti,model_fn);
...
function [rr, mstats] = model_rrate(bp, params, bp_idcs, ds, dstats, R, iti, model_fn)
...
rr = rrate(mstats, dstats, R, iti);


function rr = rrate(mstats, dstats, R, iti)
...
rr = (mean_pc*R(1) + (1-mean_pc)*R(2)) / (mean_rt+iti);
