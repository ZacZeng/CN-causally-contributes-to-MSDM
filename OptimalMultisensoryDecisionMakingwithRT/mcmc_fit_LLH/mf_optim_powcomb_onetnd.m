function [ms, biases] = mf_optim_powcomb_onetnd(d, p)
%% returns the model predictions for given subject data and parameters
%
% this model impements the optimal predictions, assuming k's and bounds to
% relate to coherence by
...
% Furthermore, it uses a single, shared non-decision time.
%
...
% pl
%
% resulting in 2 x coh_num + 12 parameters.
%
...
if nargin < 2
    ...
    prev_p = load(['fit_data' filesep 'mf_optim_powcomb_' d.subj_id '.mat']);
    assert(c_num == length(prev_p.cohs));
    prev_p = prev_p.best_p;
    assert(length(prev_p) == 2*c_num+14);
    tnd_ini = max(0,mean(prev_p(10:12)));
    ms = struct();
    ms.p_min = [0.0001  0 0.001   0 0.00001 0.001   0 0.001   0   0 (-5*ones(1,2*c_num+1))   0];
    ms.p_max = [ 10000 50    10  10    1000    10 200    10  10   1  (5*ones(1,2*c_num+1)) 0.2];
    ms.p_ini = [prev_p(1:9) tnd_ini prev_p(13:end)];
    ms.p_w   = [    50  1  0.05 0.5      20  0.03  10  0.05 0.5 0.2    (ones(1,2*c_num+1)) 0.1];
    ms.p_names = cat(2,...
...
tnds = ones(1,3)*p(10);
biases = struct('vis', p(11:(c_num+10)), 'vest', p(c_num+11), ...
    'comb', p((c_num+12):(2*c_num+11)));
pl = p(2*c_num+12);
