function [ms, biases] = mf_nocuetempw_powsep(d, p)
%% returns the model predictions for given subject data and parameters
%
% this model assumes that neither temporal nor across-cue weighting is
% performed. Rather, the (un-weighted) signals from the unimodal diffusion
% models are summes according to x_vest = sqrt(2)*(x_vis + x_vest). It
% assumes the k's and bounds to depend on coherence by 
...
% b_comb (8), g_comb (9), gam_comb (10)
% tnd_vis (11), tnd_vest (12), tnd_comb (13)
...
if nargin < 2
    ...
    ms.p_ini = [  3000 45  1.50   3   5      40  0.25   1  0.30   7 0.2 0.2 0.2     zeros(1,2*c_num+1)   0];
    ms.p_w   = [   100  1  0.10 0.5 0.5      20  0.03   1  0.05 0.5 0.2 0.2 0.2    (ones(1,2*c_num+1)) 0.1];
    ms.p_names = cat(2,...
...
for c_idx = 1:c_num
    % this a_comb causes the effective drift rate to be given by
    % a_comb(c_idx,:)*k_comb = sqrt(2)*sin(h)*(k_vis*|a_vis|+k_vest*|a_vest|)
    a_comb(c_idx,:) = ...
        sqrt(2)*(a_vis*k_vis(c_idx)/k_comb(c_idx)+a_vest*k_vest/k_comb(c_idx));
end
...
% compute per k
for k_idx = 1:k_num
    % drift is modulated by time course
    [g1, g2] = ddm_rt_dist(a*ks(k_idx), bound, delta_t, t_max, 'mnorm', 'yes');
    % compute stats
