function [ms, biases] = mf_notempw(d, p)
...
% combining the information optimally across cues.
%
% For initialisation, the model assumes to find the fits for
% mf_notempw_powcomb in the data folder. It uses those to find adequate
...
if nargin < 2
    cs = d.cohs;
    c_num = length(cs);
    % load best-found parameters from notempw_powcomb fit for initialisation
    prev_p = load(['fit_data' filesep 'mf_notempw_powcomb_' d.subj_id '.mat']);
