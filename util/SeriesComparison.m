...
addOptional(paras,'SEM',1);  % SEM (for conventional mean) or STD (for bootstrap)
% addOptional(paras,'Border',[1600, -350]);
addOptional(paras,'Border',[2000, -600, 200, -200,1000]);
% Changed by ZZ 20210506
addOptional(paras,'Gap',100);
addOptional(paras,'YLim',[]);
...
if ~iscell(ys) % Only one j (old version)
    ...
    % Config
    %     border = [2500, -2500];
    align_size = size(ts,2);
    time_markers = ts{align_size};   % Changed by ZZ 20210510
    %     time_markers = ts{4};   % Changed by ZZ 20210506
    
    
    % Plot range
    % Constrain time range to plot population PSTH  ZZ 20201229
    % align with stim_onset: end at 1600ms
    % align with sac_onset: begin at -350ms
    % Changed by ZZ 20210506
    plot_range{1} = -inf <= ts{1} & ts{1} <= border(1);
    offset{1} = 0;
    %     plot_range{2} = border(2) <= ts{2} & ts{2} <= inf;
    if length(ts) > 2   % Add if by ZZ @ 20220402
        plot_range{2} = border(2) <= ts{2} & ts{2} <=border(3);
        offset{2} = border(1) + gap + -border(2);
    end
    
    if length(ts) > 3   % Add if by ZZ @ 20220402
        plot_range{3} = border(4) <= ts{3} & ts{3} <=border(5);
        offset{3} = offset{2} +border(3) + gap + -border(4);
    end
    ...
    marker_for_time_markers{3} = {'-'};
    
    %     for j = 1:2
    for j = 1:length(time_markers)
        num_marker = size(time_markers{j}, 2);
        for tt = 1:num_marker
    ...
    if length(time_markers)>2   % Add if by ZZ @20220402
        patch([offset{2}+border(3) offset{2}+border(3) offset{2}+border(3)+gap offset{2}+border(3)+gap],...
            [min(ylim) max(ylim) max(ylim) min(ylim)],'w','edgecolor','w','linew',3)
    end
