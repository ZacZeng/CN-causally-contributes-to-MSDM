...
elseif ndims(data) == 5 && size(data,3)==2   % Added by ZZ @20230807, add  modality variable
        % different stimuli in different colours and binary condition as
    % solid/dashed
    numOfStimuli = size(data, 2) * size(data, 4);   % 5 headings and 3 conditions
    colors = lines(numOfStimuli);

    for st = 1: size(data, 4)
        for f=1:size(data, 2)
            plot(time, squeeze(data(1, f, 1, st, :)), 'color', colors(f + (st-1)*size(data, 2),:), 'LineWidth', 2)
            plot(time, squeeze(data(1, f, 2, st, :)), '--', 'color', colors(f + (st-1)*size(data, 2),:), 'LineWidth', 2)
        end
    end
